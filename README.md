...
- **チャンネルから探す**: お気に入りチャンネルを登録し、年月カレンダーからその月に公開された動画を取得。新しい順／古い順／再生回数順で並び替え（YouTube Data API v3）
...
- **タイムラインのホイール操作**: ホイールでズーム、Shift+ホイールで左右パン（メインタイムライン・波形連動）
- **盛り上がりナビゲーション**: 盛り上がり箇所へ前後ジャンプ（`,` / `.`）＋タイムライン・オーバービューにマーカー表示（クリックで候補範囲設定）
- **FFmpeg エクスポート**: トリミング領域を MP4 として書き出し（再エンコードなしの `-c copy`）
- **メタデータ保存**: クリップの情報を JSON として保存（ファイル名テンプレートで柔軟な命名）
- **メタデータ欄の簡素化**: 自動生成され手動編集しない値（Video ID / ファイル名 / クリップURL）は編集画面から非表示（裏で自動生成は継続、出力名は読み取り専用プレビューで確認）
...
- **キーボードショートカット**: カスタマイズ可能なショートカット登録・編集。編集画面に `?` トグルで常時参照できる凡例を表示
